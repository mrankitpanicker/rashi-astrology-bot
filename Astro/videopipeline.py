...
import config
import utils # For run_ffmpeg_command, format_ass_time, get_wav_duration
import whisper
import re
...
# ================================================================
# 1. VIDEO SEGMENT GENERATION CORE
# ================================================================
...
"""
    ...
    while word_pointer < n:
        ...
        for i in range(chunk_start_index, chunk_end_index):
            ...
            if i == chunk_start_index:
                # Delay from Dialogue Start to the word's actual start
                delay_from_start_s = (
    ...
    cmd = [
        "ffmpeg", "-y", 
        "-i", str(rashi_video_input.resolve()), # Input 1 (Video)
        "-i", str(audio_path.resolve()),       # Input 2 (Audio)
        "-vf", ass_filter_arg,                 # Video filter
        "-filter_complex", 
        f"[1:a]adelay={int(config.SUBTITLE_START_DELAY_SECONDS * 1000)}|{int(config.SUBTITLE_START_DELAY_SECONDS * 1000)}[a]", # Audio filter: Add 4s silence
        "-map", "0:v:0", "-map", "[a]",        # Map streams
        "-t", f"{final_output_duration:.3f}",  # Duration control
    ...
    try:
        utils.run_ffmpeg_command(cmd, f"Video Generation: {rashi}")
...
def final_video_merge(all_rashi_video_paths: List[Path]) -> Path | None:
    """
    Merges all individual Rashi video segments into one final video using FFmpeg concat demuxer.
    Returns the path to the final video or None on failure.
    """
    if len(all_rashi_video_paths) <= 1:
        print("Final merge skipped: 0 or 1 videos generated.")
        return all_rashi_video_paths[0] if len(all_rashi_video_paths) == 1 else None

    print("\n🎬 Starting final merge of all Rashi videos...")
    
    # 1. Create the concatenation list file
    concat_list_file = config.tmp_dir / "final_concat_list.txt"
    with open(concat_list_file, "w", encoding="utf-8") as f:
        for path in all_rashi_video_paths:
            f.write(f"file '{path.as_posix()}'\n")

    final_output_video = (
        config.FINAL_VIDEO_BASE_DIR / f"Final_Rashifal_{config.date_str_format}.mp4"
    )
    ...
    merge_cmd = [
        ...
        "-c", "copy",  # Fast and lossless merging
